	...
	public static function form( $atts ) {
		...
		if ( ! is_user_logged_in() ) {
			...
			if ( ! current_user_can( $current_user_capability ) ) {
				...
				return apply_filters( 'ur_register_pre_form_message', '<p class="alert" id="ur_register_pre_form_message">' . sprintf( __( 'Bạn đã đăng nhập với tên %1$1s. %2$2s', 'user-registration' ), '<a href="#" title="' . esc_attr( $display_name ) . '">' . esc_html( $display_name ) . '</a>', '<a href="' . wp_logout_url( $current_url ) . '" title="' . __( 'Đăng xuất tài khoản này.', 'user-registration' ) . '">' . __( 'Đăng xuất', 'user-registration' ) . '  &raquo;</a>' ) . '</p>', $user_ID );
